...
describe("Scenario Management Tools", () => {
  ...
  describe("Tool Registration", () => {
    ...
    test("should have correct tool configurations", () => {
      ...
      expect(tools.get("list-scenarios").description).toContain(
        "List and search Make.com scenarios with advanced filtering options",
      );
      expect(tools.get("get-scenario").description).toContain(
        "Get detailed information about a specific Make.com scenario",
      );
      expect(tools.get("create-scenario").description).toContain(
        "Create a new Make.com scenario with optional configuration",
      );
      expect(tools.get("update-scenario").description).toContain(
        "Update an existing Make.com scenario configuration",
      );
      expect(tools.get("delete-scenario").description).toContain(
        "Delete a Make.com scenario with safety checks and force options",
      );
      expect(tools.get("clone-scenario").description).toContain(
        "Clone an existing Make.com scenario with a new name",
      );
      expect(tools.get("run-scenario").description).toContain(
  ...
  describe("list-scenarios Tool", () => {
    ...
    describe("API Interaction", () => {
      test("should make correct API call with all parameters", async () => {
        const tool = (server as any).tools.get("list-scenarios");
        await tool.execute(validListArgs, {
          log: mockLog,
          reportProgress: mockReportProgress,
        });
      ...
      test("should handle successful API response", async () => {
        const tool = (server as any).tools.get("list-scenarios");
        const result = await tool.execute(validListArgs, {
          log: mockLog,
          reportProgress: mockReportProgress,
        });
  ...
  describe("get-scenario Tool", () => {
    ...
    describe("API Interaction", () => {
      ...
      test("should fetch blueprint when requested", async () => {
        const tool = (server as any).tools.get("get-scenario");
        const result = await tool.execute(validGetArgs, {
          log: mockLog,
          reportProgress: mockReportProgress,
        });

        expect(mockApiClient.get).toHaveBeenCalledWith(
          "/scenarios/scn_123/blueprint",
        );
  ...
  describe("create-scenario Tool", () => {
    ...
    describe("API Interaction", () => {
      ...
      test("should include all optional parameters when provided", async () => {
        const tool = (server as any).tools.get("create-scenario");
        await tool.execute(validCreateArgs, {
          log: mockLog,
          reportProgress: mockReportProgress,
        });
      ...
      test("should return created scenario details", async () => {
        const tool = (server as any).tools.get("create-scenario");
        const result = await tool.execute(validCreateArgs, {
          log: mockLog,
          reportProgress: mockReportProgress,
        });
    ...
    describe("Error Handling", () => {
      test("should handle creation failure", async () => {
        ...
        await expect(
          ...
          "Failed to create scenario: Insufficient permissions",
        );
      });
    });
  });
  ...
  describe("update-scenario Tool", () => {
    ...
    describe("Input Validation", () => {
      ...
      test("should validate name length", async () => {
        ...
        await expect(
          tool.execute(
            { scenarioId: "scn_123", name: longName },
            { log: mockLog, reportProgress: mockReportProgress },
          ),
        ).rejects.toThrow();
      });
    ...
    describe("API Interaction", () => {
      ...
      test("should include all provided update fields", async () => {
        const tool = (server as any).tools.get("update-scenario");
        await tool.execute(validUpdateArgs, {
          log: mockLog,
          reportProgress: mockReportProgress,
        });
      ...
      test("should return update summary", async () => {
        const tool = (server as any).tools.get("update-scenario");
        const result = await tool.execute(validUpdateArgs, {
          log: mockLog,
          reportProgress: mockReportProgress,
        });
  ...
  describe("clone-scenario Tool", () => {
    ...
    describe("Cloning Process", () => {
      test("should fetch source blueprint first", async () => {
        const tool = (server as any).tools.get("clone-scenario");
        await tool.execute(validCloneArgs, {
          log: mockLog,
          reportProgress: mockReportProgress,
        });

        expect(mockApiClient.get).toHaveBeenCalledWith(
          "/scenarios/scn_source/blueprint",
        );
      });
      ...
      test("should return cloning result", async () => {
        const tool = (server as any).tools.get("clone-scenario");
        const result = await tool.execute(validCloneArgs, {
          log: mockLog,
          reportProgress: mockReportProgress,
        });
  ...
  describe("run-scenario Tool", () => {
    ...
    describe("Input Validation", () => {
      ...
      test("should validate timeout range", async () => {
        ...
        await expect(
          tool.execute(
            { scenarioId: "scn_123", timeout: 301 },
            { log: mockLog, reportProgress: mockReportProgress },
          ),
        ).rejects.toThrow();
      });
    ...
    describe("Execution Workflow", () => {
      ...
      test("should poll for completion when wait=true", async () => {
        const tool = (server as any).tools.get("run-scenario");
        const result = await tool.execute(validRunArgs, {
          log: mockLog,
          reportProgress: mockReportProgress,
        });

        expect(mockApiClient.get).toHaveBeenCalledWith(
          "/scenarios/scn_123/executions/exec_123",
        );
    ...
    describe("Progress Reporting", () => {
      test("should report progress during execution monitoring", async () => {
        const tool = (server as any).tools.get("run-scenario");
        await tool.execute(validRunArgs, {
          log: mockLog,
          reportProgress: mockReportProgress,
        });
  ...
  describe("Integration Tests", () => {
    test("should handle complete scenario lifecycle", async () => {
      ...
      await updateTool.execute(
        { scenarioId: "scn_lifecycle", active: true },
        { log: mockLog, reportProgress: mockReportProgress },
      );

      // Run scenario
      await runTool.execute(
        { scenarioId: "scn_lifecycle", wait: false },
        { log: mockLog, reportProgress: mockReportProgress },
      );

      // Force delete active scenario
      await deleteTool.execute(
        { scenarioId: "scn_lifecycle", force: true },
        { log: mockLog, reportProgress: mockReportProgress },
      );
  ...
  describe("Performance and Edge Cases", () => {
    ...
    test("should handle Unicode scenario names", async () => {
      ...
      mockApiClient.post.mockResolvedValue({
        success: true,
        data: { id: "scn_unicode", name: unicodeName },
      });
