...
try {
  ...
  console.log(`✅ Core tools: ${coreConfig.coreToolCategories.length} categories`);
  console.log(`✅ Analytics tools: ${analyticsConfig.analyticsToolCategories.length} categories`);
  
  // Test that the configurations are properly separated
  const totalCategories = coreConfig.coreToolCategories.length + analyticsConfig.analyticsToolCategories.length;
  ...
  if (intersection.size === 0) {
    ...
    console.log(`⚠️  Found ${intersection.size} overlapping categories: ${Array.from(intersection).join(', ')}`);
  }
  ...
  const analyticsServerModule = await import('./src/servers/analytics-server.js');
  ...
  console.log(`📱 Core Server Categories (${coreConfig.coreToolCategories.length}):`, coreConfig.coreToolCategories.join(', '));
  console.log(`📊 Analytics Server Categories (${analyticsConfig.analyticsToolCategories.length}):`, analyticsConfig.analyticsToolCategories.join(', '));
  ...
  console.log('\n🔗 Available Commands:');
  console.log('🚀 npm run dev:core      - Start Core Operations Server (port 3000)');
  console.log('🚀 npm run dev:analytics - Start Analytics & Governance Server (port 3001)');  
