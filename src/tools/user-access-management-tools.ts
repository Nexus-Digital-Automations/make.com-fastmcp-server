...
const _OrganizationCreateSchema = z.object({
  name: z
    .string()
    .min(1)
    .max(100)
    ...
    execute: async (args, { log }) => {
      ...
      try {
        ...
        const formattedOrgs = await Promise.all(
          organizations.map(async (org: MakeOrganization) => {
            let teamCount = 0;
            let userCount = 0;
            ...
            return {
              ...
              name: org.name,
              role:
                (org as MakeOrganization & { role?: string }).role || "member",
