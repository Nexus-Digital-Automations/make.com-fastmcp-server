  ...
  return {
    ...
    execute: async (args: unknown, context): Promise<string> => {
      ...
      try {
        ...
        const response = await fetchScenariosWithParams(
          apiClient,
          params,
          reportProgress,
        );
...
function parseListScenariosArgs(args: unknown): {
  ...
  try {
    const result = ScenarioFiltersSchema.parse(args);
    return result;
  } catch (error) {
...
function buildQueryParameters(argsTyped: {
  ...
  if (argsTyped.teamId) {
    params.teamId = argsTyped.teamId;
  }
  if (argsTyped.folderId) {
    params.folderId = argsTyped.folderId;
  }
  if (argsTyped.search) {
    params.q = argsTyped.search;
  }
  if (argsTyped.active !== undefined) {
    params.active = argsTyped.active;
  }
...
function formatScenariosResponse(
  ...
  return {
    ...
    pagination: {
      ...
      hasMore:
        ((metadata as { total?: number })?.total || 0) >
        (argsTyped.offset || 0) + (argsTyped.limit || 20),
    },
