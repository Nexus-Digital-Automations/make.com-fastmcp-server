...
export function createGetScenarioTool(context: ToolContext): ToolDefinition {
  ...
  return {
    ...
    execute: async (args: unknown, context): Promise<string> => {
      ...
      let typedArgs;
      try {
        typedArgs = ScenarioDetailSchema.parse(args);
      } catch (error) {
        const errorMessage =
      ...
      log?.info?.("Getting scenario details", {
      ...
      try {
        ...
        if (!response.success) {
          throw new UserError(
            `Failed to get scenario: ${response.error?.message}`,
          );
        }
        ...
        if (typedArgs.includeExecutions) {
          const executionsResponse = await apiClient.get(
            `/scenarios/${typedArgs.scenarioId}/executions`,
            {
              params: { limit: 10 },
            },
          );
