...
export function createUpdateScenarioTool(context: ToolContext): ToolDefinition {
  ...
  return {
    ...
    execute: async (args: unknown, context): Promise<string> => {
      ...
      let typedArgs;
      try {
        typedArgs = UpdateScenarioSchema.parse(args);
      } catch (error) {
        const errorMessage =
      ...
      try {
        ...
        if (typedArgs.name !== undefined) {
          updateData.name = typedArgs.name;
        }
        if (typedArgs.active !== undefined) {
          updateData.active = typedArgs.active;
        }
        if (typedArgs.blueprint !== undefined) {
          updateData.blueprint = typedArgs.blueprint;
        }
        if (typedArgs.scheduling !== undefined) {
          updateData.scheduling = typedArgs.scheduling;
        }
        ...
        if (!response.success) {
          throw new UserError(
            `Failed to update scenario: ${response.error?.message}`,
          );
        }
        ...
        log?.info?.("Scenario updated successfully", {
