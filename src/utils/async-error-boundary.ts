...
export class AsyncErrorBoundary {
  ...
  async execute<T>(
    operation: () => Promise<T>,
    context: { operation: string; metadata?: Record<string, any> } = { operation: 'unknown' }
  ): Promise<T> {
    ...
    try {
      this.componentLogger.warn('All retry attempts exhausted, executing fallback', {
        operation: context.operation,
  ...
  unregisterResource(resourceName: string): void {
    ...
    AsyncErrorBoundary.globalResources.delete(`${this.options.name}:${resourceName}`);
  }
  ...
  async cleanup(): Promise<void> {
    ...
    const priorityOrder: ResourceCleanup['priority'][] = ['high', 'medium', 'low'];
  ...
  static async shutdown(): Promise<void> {
    ...
    const shutdownPromises = Array.from(AsyncErrorBoundary.shutdownHandlers).map(async (handler) => {
      try {
        await handler();
      } catch (error) {
   ...
   */
  static create(name: string, options: Omit<ErrorBoundaryOptions, 'name'> = {}): AsyncErrorBoundary {
...
export const databaseBoundary = AsyncErrorBoundary.create('DatabaseOperations', {
  retryAttempts: 3,
  retryDelayMs: 500,
