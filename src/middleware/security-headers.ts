...
export class SecurityHeadersManager {
  ...
  private helmetInstance: (
    req: HttpRequest,
    res: HttpResponse,
    next: NextFunction,
  ) => void = () => {};
  private csrfUtilities: {
    generateCsrfToken: (req: HttpRequest, res: HttpResponse) => string;
  ...
  private setupHelmet(): void {
    this.helmetInstance = helmet({
      contentSecurityPolicy: this.buildCSPConfig(),
      hsts: this.buildHSTSConfig(),
      ...
      crossOriginEmbedderPolicy: this.buildCOEPConfig(),
      hidePoweredBy: true,
  ...
  private buildCSPConfig(): unknown {
    return {
      directives: {
        ...
        imgSrc: this.buildImgSrcDirectives(),
        connectSrc: this.buildConnectSrcDirectives(),
  ...
  private buildImgSrcDirectives(): string[] {
    return [
      "'self'",
      "data:",
      "https:",
  ...
  private buildConnectSrcDirectives(): string[] {
    return [
      "'self'",
      "https://api.make.com",
      "https://eu1.make.com",
  ...
  private setupCSRF(): void {
    if (this.config.enableCSRF) {
      ...
      this.csrfUtilities = doubleCsrf({
        ...
        getSessionIdentifier: (req: HttpRequest) =>
        ...
        if (err) {
          logger.warn("CSRF protection triggered", {
            ...
            userAgent: ((): string => {
              ...
              const userAgent =
                typeof ua === "string" ? ua : Array.isArray(ua) ? ua[0] : "";
  ...
  private applyCustomSecurityHeaders(
    req: HttpRequest,
    res: HttpResponse,
  ): void {
  ...
  public isSensitiveEndpoint(path: string | undefined): boolean {
    ...
    return path
      ? sensitivePatterns.some((pattern) => path.startsWith(pattern))
      : false;
  }
  ...
  public getContentTypeValidation(): (
    ...
    return (req: HttpRequest, res: HttpResponse, next: NextFunction) => {
      ...
      if (req.method && ["POST", "PUT", "PATCH"].includes(req.method)) {
        ...
        if (
          contentTypeString &&
          !allowedTypes.some((type) => contentTypeString.includes(type))
        ) {
  ...
  private isSecurityRelevantRequest(req: HttpRequest): boolean {
    ...
    return (
      securityEndpoints.some((endpoint) => req.path?.startsWith(endpoint)) ||
      req.method !== "GET" ||
...
export function validateSecurityHeaders(req: HttpRequest): {
  valid: boolean;
  issues: string[];
} {
  const issues: string[] = [];
