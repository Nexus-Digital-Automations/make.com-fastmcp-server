...
export class IntegratedSecurityManager {
  ...
  public getAuthSecurityMiddleware(): MiddlewareFunction[] {
    ...
    if (
      ...
      this.config.rateLimiting.tiers.auth
    ) {
  ...
  public getSensitiveEndpointMiddleware(): MiddlewareFunction[] {
    ...
    if (
      this.config.rateLimiting.enabled &&
      this.config.rateLimiting.tiers.sensitive
    ) {
  ...
  public getWebhookMiddleware(): MiddlewareFunction[] {
    ...
    if (
      this.config.rateLimiting.enabled &&
      this.config.rateLimiting.tiers.webhooks
    ) {
  ...
  public getSecurityStatus(): {
    ...
    const status = this.buildSecurityComponentStatus();
    const issueCount = this.countSecurityIssues(status);
    status.overall = this.determineOverallHealth(issueCount);
    return status;
  }

  /**
   * Build security component status object
   */
  private buildSecurityComponentStatus(): {
    rateLimiting: Record<string, unknown>;
    ddosProtection: Record<string, unknown>;
    circuitBreakers: Record<string, unknown>;
    monitoring: Record<string, unknown>;
    ...
    return {
      rateLimiting: this.config.rateLimiting.enabled
        ? rateLimitManager.getSystemStatus()
        : { enabled: false },
      ddosProtection: this.config.ddosProtection.enabled
  ...
  /**
   * Count security issues across components
   */
  private countSecurityIssues(status: Record<string, unknown>): number {
    let issues = 0;

    if (this.isRateLimitingIssue(status.rateLimiting)) {
      issues++;
    }

    if (this.isMonitoringRiskIssue(status.monitoring)) {
      issues++;
    }
   ...
   */
  private isRateLimitingIssue(
    rateLimitingStatus: Record<string, unknown>,
  ): boolean {
    return (
      this.config.rateLimiting.enabled &&
      "redisConnected" in rateLimitingStatus &&
      !rateLimitingStatus.redisConnected &&
      this.config.rateLimiting.redis.enabled
    );
  }

  /**
   * Check if there's a monitoring risk issue
   */
  private isMonitoringRiskIssue(
    monitoringStatus: Record<string, unknown>,
  ): boolean {
    return (
      this.config.monitoring.enabled &&
      "currentRiskScore" in monitoringStatus &&
      Number(monitoringStatus.currentRiskScore) > 70
    );
  }

  /**
   * Determine overall health based on issue count
   */
  private determineOverallHealth(issueCount: number): string {
    if (issueCount === 0) {
      return "healthy";
...
export function validateSecurityConfiguration(): {
  ...
  if (
    ...
    config.rateLimiting.redis.enabled &&
  ...
  if (
    ...
    config.errorSanitization.developmentMode
  ) {
  ...
  if (!config.monitoring.enabled) {
    warnings.push(
      "Security monitoring disabled - reduced visibility into threats",
    );
  }
