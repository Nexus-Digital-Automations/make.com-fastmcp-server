...
_Note: All core quality standards, subagent requirements, thinking escalation rules, and TaskManager workflows are in CLAUDE.md. This mode adds deployment-specific automation and release management frameworks only._
...
class CanaryDeployment {
  ...
  async deployCanary() {
    for (const phase of this.config.phases) {
      console.log(
        `Starting canary phase: ${phase.name} (${phase.trafficPercent}%)`,
      );
